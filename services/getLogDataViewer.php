...
include('../admin/inc/services/getTimeString.php');
...
if ($ids) {
	...
	if ($logs) {
            ...
            $logData[] = array(
                "rut" => isNull($logs['rut']),
                "date" => getDatetimeDate($logs['datetime']),
                "time" => getDatetimeTime($logs['datetime']),
                "description" => isNull($logs['description']),
                "zone" => isNull($logs['zone']),
                "module" => isNull($logs['module']),
                "submodule" => isNull($logs['submodule']),
                "username" => isNull($logs['username']),
                "waitingStart" => getDatetimeTime($waitingStart),
                "attentionStart" => getDatetimeTime($attentionStart),
                "attentionFinish" => getDatetimeTime($attentionFinish),
                "waitingTime" => isNull($waitingTime),
                "attentionTime" => isNull($attentionTime),
                "secondDescription" => isNull($secondDescription)
            );
...
function getDatetimeTime($datetime) {
    if ($datetime == null) {
        return isNull($datetime);
    } else {
        $timestamp = strtotime($datetime);
        return $time = date('G:i:s', $timestamp);
    }    
}
