	...
	{	
		...
		{			
				...
				for($j=$iDate-1;$j<=$eDate;$j++)
				{
					...
					else
					{
						...
						while($j==$number && ($time<=date("H:i", strtotime($this->event[$id]['time'])) && $nextTime>date("H:i", strtotime($this->event[$id]['time']))) && $id < count($this->event) ) 
						{
							...
							echo '<div id="'.$this->event[$id]['id'].'" class="drag t'.$priority['priority'].'" '.$onClick.' align="left" font-family: Verdana font-size: 10px;">'.$this->event[$id]['title'].'</div>';
			...
			/*}else
			{
				...
				{
					$franja=1;
					echo '<tr ><td height="50px" colspan="8" style="background:#336699" > ';
					echo '</td></tr>';
				}
				...
				<div id = "wrapper">
					...
					<div class="tab_container">
						<div class="tab_content">
							<?if(isset($_REQUEST['id'])){?>
								<iframe id="iframeCalendar" src="iframeCalendar.php?view=<? echo $this->vista;?>&id=<? echo $_REQUEST['id'];?>&date=<? echo $_REQUEST['actualDate'];?>&type=<? echo $_REQUEST['type']?>&filter=<? echo $_REQUEST['filter']?>" name="main" width="100%" SCROLLING="AUTO"></iframe>
							<?}else
							  {
