...
$wado = "http://$server:8080/wado?requestType=WADO";
...
$imagesTxt = "images=";
$images = $study->doSql("SELECT * FROM instance WHERE series_fk=$series_pk ORDER BY LPAD(inst_no,10,'0')");
$i=0;
do
{
	$dicomPath = $files->doSql("SELECT filepath FROM files WHERE instance_fk=".$images['pk']." ORDER BY created_time");
