		...
		ajax.onreadystatechange=function() {
				if (ajax.readyState==4) {
						...
						if (answer)
						{
							...
							xmlhttp2.open("POST","deleteCalendar.php", true);
							xmlhttp2.onreadystatechange=function() {
								if (xmlhttp2.readyState==4) {
								}
							}
							xmlhttp2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
							xmlhttp2.send("remove="+id);
	...
	window.onload=function(){
		...
		REDIPS.drag.myhandler_clicked=function(){
			...
			{	
				...
				otroClick = REDIPS.drag.obj.parentElement.id
				...
				ajax_.onreadystatechange=function() {
					if (ajax_.readyState==4) {
							...
							if(resp[0]==0)
							{
								...
								{ ?>
									var remove ='<a onclick="deleteEvent('+ "'" +lastClickId+ "'" +', '+ "'" +otroClick+ "'" +');"><img class="imgEdit-Delete" src="../../../images/delete_s.png"/></a>';
								<? }
								else{ ?>
									var remove='';
								<? } ?>
				...
				if(tcn!=sco) //verificamos que no se este moviendo en la misma celda
				{
				   ...
				   ?>
					...
					else
					{
						...
						REDIPS.drag.source_cell.innerHTML = REDIPS.drag.obj_old.outerHTML;
						REDIPS.drag.target_cell.innerHTML = '';
